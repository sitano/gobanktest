...
func Test_CompareShouldBeCS(t *testing.T) {
	u1 := &User{Name: "User1", Purse: 1}
	u2 := &User{Name: "User1", Purse: 1}
...
func Test_CompareShouldDifferPurses(t *testing.T) {
	u1 := &User{Name: "User1", Purse: 1}
	u2 := &User{Name: "User1", Purse: 2}
...
func Test_STLoadOfMissingUserShouldFail(t *testing.T) {
	...
	if user, err := db.Load("User1"); user != nil {
		t.Fatal("DB should not load non existent user")
	} else if err.Error() != "There is no such user User1" {
...
func Test_STSaveShouldRewriteExistingUser(t *testing.T) {
	...
	u1 := &User{Name: "User1", Purse: 1}
	u2 := &User{Name: "User2", Purse: 3}
	u3 := &User{Name: "User1", Purse: 2}
	...
	if u4, err := db.Load(u1.Name); err != nil {
		t.Fatal("DB failed to load an existent user", u1.Name, err)
	...
	if u5, err := db.Load(u2.Name); err != nil {
		t.Fatal("DB failed to load an existent user", u2.Name, err)
...
func Test_STLoadShouldSeeExistedUser(t *testing.T) {
	...
	if u11, err := db.Load(u1.Name); err != nil {
...
func Test_STTXPutShouldNotRewrite(t *testing.T) {
	...
	if u3, err := db.Load(u1.Name); err != nil {
		t.Fatal("DB failed to load an existent user", u1.Name, err)
...
func Test_STTXChangeShouldNotSaveUnexpectedValue(t *testing.T) {
	...
	if err := db.Transaction().Change(u1.Name, 1, 2); err == nil {
	...
	if err := db.Transaction().Change(u1.Name, 1, 1); err != nil {
...
func Test_STTXChangeShouldAllowNegative(t *testing.T) {
	...
	u1 := &User{Name: "User1", Purse: 0}

	if err := db.Transaction().PutIfAbsent(u1); err != nil {
		t.Fatal("DB tx failed to create a user", u1, err)
	}

	if err := db.Transaction().Change(u1.Name, -1, 0); err != nil {
		t.Error("DB tx change must not change user with unexpected value", u1, err)
	}
